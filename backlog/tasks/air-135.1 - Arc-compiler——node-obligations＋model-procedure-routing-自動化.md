...
AIR-134 的重發／收斂版。把使用者反覆提醒的 procedure 與 model 流程改成 Marshal 可執行的 node-transition contract：從 card tree 正規化 ArcSpec，產穩定 ArcPlan（單調版本號＋內容 hash），dispatch 前按 runtime entitlement/window JIT resolve 出 DispatchSlice——必攜 revert 預算餘額、sink/accept、budget context（敞口帽與用量帽分欄）、role-dependent read-set（review/verify 禁逐字繼承 implementer reasoning；Intent Review slice 只給 entry 六欄＋final artifact、排除中間鏈）。歧義、雙 authoritative 值、priced-autonomy 必要欄位缺場一律 fail-loud——缺欄不得派工。恢復是 Marshal invariant：work unit 錨 card node id、runtime tail 缺場或 ledger/artifact 矛盾 fail-loud、已完成 work unit 不重做。另產 LLM 使用規劃 artifact：依時間窗口＋coding plan entitlement 事前規劃各 model/binding 分配（旅程④）。核心成功條件不是產生手冊，而是 user 只給方向也能走對方法。orchestration 可靠性面（main-seat、artifact-first、collection、checkpoint）已拆至 AIR-135.7，本卡只把它們編譯成 slice 欄位；同 repo 不把 handoff 當流程 node。
