...
設計 AIR-135 的跨 repo 落地 contract。Backlog.md source 查證後採 qualified text identity：每 repo 保有自己的 canonical card tree；對應卡以 references[]＋Description 逐字 token 對應（https 僅可點捷徑非 identity），parentTaskId 與 dependencies 維持 repo-local。ownership：ai-guide 擁 Development Arc semantics/policy/Marshal obligations/projection contract；SouthChariot 擁人機 control surface（唯讀 canonical、產 derived projection/cache）、不另存第二套 lifecycle truth。開卡協議 reverse-first：contract 固定後 SC 先建 repo-local card 寫 reverse token 指回 AIR-135.5，再回 ai-guide 補 forward token；絕對本地路徑禁作 identity。跨 repo 發送＝outward 憲法級逐次授權（0919 commit 委任不及於此）：草案八欄（target/source identity／verdict 摘要＋座標／artifact pointers＋hash／expected response／correlation id／context delta／expiry）組裝後走 transport 三路分流——活人看且時效→SC-16 router（四態 receipt）、durable 留檔→卡面 token 回寫、要對端幹活→bridge task（派工非通知）；單一發送路徑、草案可排隊禁批量發送。禁自建跨 repo session 目錄（registry 只存期望）；parties 多方化：本側 ai-guide＋delegate-bridge，對端 mosaic＋SC；跨 repo liveness／pending 投影可追溯 authoritative owner（仲裁歸 135.3、Settle 驅動歸 135.7）。

```mermaid
flowchart TB
  AG["ai-guide<br/>semantics/policy/orchestration owner"] <-->|"x-arc token<br/>reverse-first 開卡"| SCS["SouthChariot<br/>control surface（唯讀 canonical）"]
