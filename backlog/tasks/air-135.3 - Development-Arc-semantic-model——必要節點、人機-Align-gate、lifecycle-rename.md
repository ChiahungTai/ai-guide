...
重新定義開發流程的 semantic model，而不是把現行 commands/skills 升格成 lifecycle nodes。必要節點＝entry Align（human，六欄交代，缺任一不得進 Shape）→ Shape/Plan（human，「SPEC 好了」手勢）→ Build/Verify（自動；Verify 保留獨立判斷軸）→ exit Align（human，終場六塊＋intent 雙證：弧內自報 intent-diff＋獨立 Intent Review 腿——fresh、排除中間鏈、問「仍解原本問題嗎」；觸發閘＝standard＋full 且敞口過半／有 invalidated row／動過跨卡契約）→ Settle（human，批量裁決）。Marshal 擁 phase transition 驅動與 commit 條件委任（bi/tri＋post-build 過關即授權）；打斷採 priced-autonomy 語彙——強 outward／真分岔／自我擴權／預算超支才停；pending 台帳是未決人類裁決唯一住處（含批量浮出類 evidence-contradicts-intent／assumption-invalidated，晨間裁決消化）。可觀察性＝Marshal cross-cutting：slice 邊界 pull-only checkpoint，永不中途推播提問、唯一合法推播＝破損警報。Marshal-owned mechanics（WT/session/model routing/retry/同 repo handoff）不進人類心智模型；node 必要性以 session audit＋gate-position 測試驗證；rename／deprecation 最終 verdict 歸 parent S7。
