...
【0925 sweep 掃描邊界註記（W5 落地後顯影，user 問答觸發）】sweep 覆蓋面＝ai-guide 主樹單 repo：①期望台帳＝主樹 .agent-tmp/dispatch-expectations.json（plist cd 死主樹；REPO_ROOT=腳本 parents[1]）②實際面＝主樹＋delegate-bridge 兩 state root 的 jobs.json。已知限制：bridge jobs.json 是 per-workspace——卡 WT（wt-open）派的 job 帳落各 WT 的 .delegate-bridge/，主樹 sweep 的 liveness 對接看不到（0925 實證：主樹 show 找不到 135.7 WT 的 job）。補救路徑：(a) 接線紀律＝marshal 派工後統一 register 期望進主樹台帳（--ledger 主樹路徑），掃描面＝所有記帳 dispatch——liveness row 查不到的標 unknown 即漂移訊號（契約內）；(b) 跨 WT 巡掃升級＝另案卡。現況台帳零 register——sweep 實掃集合為空，err log 每日一行即此狀態。
<!-- SECTION:NOTES:END -->
