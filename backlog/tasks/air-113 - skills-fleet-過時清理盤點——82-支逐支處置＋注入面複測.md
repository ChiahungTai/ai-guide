...
0917 S3 Gate A（AC#3 驗收）：**PASS**——76 支全格式注入總長 18,079 chars ≤ metadataBudget 20,000（AIR-107 逆向公式機械重算；起點 82 支 ≈25,873 恆降級 name-only → wave1 desc 瘦身＋S2 減量聯合翻轉為全格式呈現，餘裕 1,921 chars）。註：格式依 AIR-107 docstring 重建，~10% 餘裕下可選跑一次 headless smoke 做最終確認。**Gate A 過＝N 判定上限確立：76 支為 metadataBudget 內最大保留集（當前 desc 形態下）**。AC#2 已 handoff mosaic 側（user 轉交，進行中）。仍 open：Gate B（confusion pairs 行為 probe——wave1 互斥句為 treatment 基線）＋python-type-gap 逐支裁。
0917 AC#2 收線＋python-type-gap 裁決（本弧最終批）：**AC#2 完成**——mosaic 側全數接入四支（`27c3eb155`，mos-107；codex discovery smoke PASS＝symlink 掃描實證，swing/frontend bi defer 項一併結清）；ai-guide 側刪 `skills/{upgrade-nt,upgrade-sj,nt-query,nt-v1-query}/`＋`_common/upgrade-flow.md`（骨架隨遷，ai-guide 消費者僅雙 skill 已驗證）；CLAUDE.md 四行刪＋AGENTS.md:113 觸發錨改遷移指針；名稱級保留（cr-query×2/sync-check-angles/voice-notification）；nt-v1 sunset 綁 mosaic v2 cutover（責任隨遷移交）。**python-type-gap 裁決**（bi panel codex+muse 一致 (b)＋5.3 裁決採納）：reference-demotion→`fix-test/python-type-gap.md`＋`python-type-gap-reference.md`（lookup 食譜同遷）；fix-test desc/when_to_use 吸收「第三方型別缺口/stubs/type:ignore」觸發；implement:23＋commit:49 斷鏈重指。**fleet 82→71**；Gate A 重算＝16,933 chars ≤ 20,000 PASS（餘裕擴至 3,067）。Gate B（confusion pairs flash probe）進行中。
0917 Gate B 完成（flash carrier×6，`​.agent-tmp/air113/gateb/summary.md`）：**confusion 臂 3/3 PASS**（降級/吸收後零誤觸——desc 互斥性成立，Gate B 主風險通過）；**headless smoke＝inject full**（真 zcode.cjs session 全格式注入——Gate A 機制閘正式確認）；positive 臂 1/3（code-review PASS；instruction-writing/fix-test 未載入直接動作）——歸因三混淆：flash carrier 判斷層、prompt-desc 詞彙距離、單發無統計＋六跑全壓 240s timeout。Actionable 落地：instruction-writing desc 補「操作指南/說明文件/給 AI 讀的文檔」觸發詞（詞彙距離與 carrier 無關、獨立成立）；fix-test 不動（desc 已明列 ruff/mypy，miss 較像 carrier 把任務當機械動作）。5.3-carrier 去混淆複跑＝optional follow-up（未排）。**AIR-113 S3 雙 gate 至此全跑完：Gate A PASS（16,933/20,000）＋Gate B 主風險通過＋發現面改善一項落地。**
<!-- SECTION:NOTES:END -->
