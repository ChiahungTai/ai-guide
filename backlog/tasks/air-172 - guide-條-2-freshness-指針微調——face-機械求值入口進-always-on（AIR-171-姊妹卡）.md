...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 條 2 尾句置換為 face 求值指針（muse/codex 收斂措辭）；判準式逐字不變；rg 驗證 guide freshness 單點＋cr-query 錨可解析
- [ ] #2 兩消費 skill 與 rules/ 零變更；bundle 尺寸無實質增長；三面 deploy/check 綠
- [ ] #3 落地 review：boundary profile receipt（instruction-writing——新增 executable face pointer 非 static-only typo）
<!-- AC:END -->
