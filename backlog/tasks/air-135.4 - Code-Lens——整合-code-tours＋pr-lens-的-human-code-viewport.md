...
- [ ] #2 code tour 與 pr-lens 共用一份最小 projection contract；同一 change 可切 narrative 與 topology/impact 視角，不重算／複製彼此已有的 core facts。**M1 卡投影視圖類型（L1 SA content view——0919 bi 校正）**：旅程②卡內容討論的視圖＝人類內容視圖五塊——①這卡在解什麼（lift Description＋SA capsule）②定什麼契約（SA:CONTRACT）③不做什麼＋承接哪些上游契約（SA:BOUNDARY＋各 direct dependency 的 SA:EXPORT 逐字 lift；深度=1、transitive 只計數＋drill-down）④影響誰＋現況（deps/rdeps/status/AC 機械算）⑤還有什麼要人裁決（pending pointers，無則明示）——ephemeral、隨卡重生成、示於 SC 整合瀏覽器；freshness＝實際 consumed blocks 的 composite digest（stale 標段級，非整頁）；直讀 canonical 卡面，禁以卡尾 PROJECTION 段為 source（projection-on-projection）、禁 renderer 爬 transitive 鏈（缺前提＝修 dependency 表達非爬鏈補洞）。事實協商中禁 code-fact 圖；SA capsule schema 歸 135.2 契約⑫-⑯，本卡定義視圖類型與 ephemeral 語義。**Code Lens view mapping 須標 Development Arc node 與觸發時刻**：entry Align 顯示卡內容事實投影；Shape exit 顯示 SPEC／計畫投影；exit Align 顯示終場 receipt、intent-diff、decision、revert／no-impact／否決介面
...
白話：畫圖給你看的工具——把卡上的事實投影成視圖（一卡一頁的正式渲染版）。
<!-- SECTION:NOTES:END -->

<!-- PROJECTION:BEGIN -->
白話：畫圖給你看的工具——把卡上的事實投影成視圖（一卡一頁的正式渲染版）。
要等：AIR-135.2、AIR-135.3
擋誰：無
現況：To Do；AC 0/5
源 hash：14aa372192d8（卡面減投影段；不符即 stale）
<!-- PROJECTION:END -->
