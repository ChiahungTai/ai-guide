...
畫圖給人看的工具——canonical facts（card tree＋repo/code facts）按需投影成人類視圖。不是 lifecycle node、不是第二 truth source。

**確認介面**

- 開卡確認＝**SC ext 點卡預覽**（卡本身即投影，ai-guide 側零平行 render）
...
- M1 卡內容視圖五塊：解什麼／定什麼契約／不做什麼＋承接上游（direct deps 深度=1，transitive 只計數）／影響＋現況（機械算）／待裁決（pending pointers）
- generator 機械投影禁手抄（P9 實證）；帶新鮮度錨，stale 自稱 stale
- 選型 manual-first，門禁句：渲染前自問消費者是誰、時刻是哪個、理解層在哪——答不出就不跑
- 執行中沉默（liveness 歸 135.7）；批量只一張汇总 receipt；SC 只消費 view contract
- pr-lens＝code 弧視圖主場（walkthrough／architecture delta／impact），卡投影與其為 Code Lens family 兄弟（slice-1＝原 AIR-130 producer，mosaic tools/prlens）
