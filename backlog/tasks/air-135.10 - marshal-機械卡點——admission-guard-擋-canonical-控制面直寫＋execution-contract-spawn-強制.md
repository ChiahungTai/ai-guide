...
<!-- SECTION:NOTES:BEGIN -->
【0924 SC 端 finding 登記（結算信 5227d7fa 條1；回執已發）】wt-open.sh:89 case pattern *[!A-Za-z0-9-]* 拒帶點子卡 id（sc-236.3/sc-231.1/sc-246.1 三卡命中）——SC 側手動 git worktree add＋自寫 identity contract 繞過。verdict＝採納：帶點子卡 id 是兩 repo 共同常態（AIR-135.x 同構），pattern 保守過頭。待開卡處理（開卡協議①——Description 待 user 確認週期）：選項 a) pattern 放寬容點（git branch 中段點號合法，禁開頭/連續兩點仍由既有檢查守）b) 文件化 sanitize 規則（branch 去點＋identity.note 存 canonical id——SC 現行繞法正典化）。SC 現行 workaround＝後者實證。
<!-- SECTION:NOTES:END -->
