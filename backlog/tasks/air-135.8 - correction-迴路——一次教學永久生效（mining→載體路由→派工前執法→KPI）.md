...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 session 級挖掘：每弧結算（post-build 鏈）機械掃 correction 候選——基礎設施種子＝corrections-weekly 的 mine_corrections.py，從週報顆粒升級為每弧；挖掘清單落 session journal 並入 pending 機制
- [x] #2 載體路由：每條 correction 按性質寫進對應載體——卡面 section（intent/procedure）／decision entity（跨卡長壽）／rule／skill（條文面，標注走 instruction 落地前審查閘）；路由表為本卡交付
- [x] #3 派工前執法：Marshal dispatch 前機械檢查未收編 correction 清單——open 項 blocking（機制可比 decisions_pending.py lint --card 模式）；執法點歸 135.7 DispatchSlice 義務
- [x] #4 KPI：同一 correction user 需重述第二次＝迴路 fail；correction 計數與重述率併入 parent S7 三欄度量
- [x] #5 與 135.2 邊界：載體契約（什麼進 decision entity vs Notes）歸 135.2 AC#3；本卡只做迴路（挖掘/路由/執法/度量）
...
【0924 批量夜 #1 實作收線】commit 700bbb1f @ ephemeral/air-135.8-impl——四步迴路落地：①挖掘 mine_corrections --since/--until 弧窗（反向窗 ap.error 擋誤用）②路由 post-build 階段 4 correction mining checkpoint（Detect→Extract→收編→路由→執法；四載體分流表主體）③執法 decisions_pending kind=correction＋lint --dispatch（精確卡匹配、correction 禁 `-`——無卡不受兩閘涵蓋）④KPI correction-kpi.jsonl detected→incorporated（obsolete 不記）。review：muse 全 diff＋codex 設計腿 approve-with-findings（零 🔴）；judge 採納 muse F1 反向窗／F2 correction 禁 -／F3 restated 跨 WT 路徑 git-common-dir 解析／G2 footer＋codex 重述 identity 原則（correction id＋原文 hash、LLM 聚類僅輔助）。機驗：28 tests＋commit 全量 2136 綠、F1/F2 spot exit 2、乾淨派工 exit 0。後續項（不擋本批，歸 KPI 讀端/135.7 批次）：G1 routed 讀端＋KPI lifecycle 細化（decided/implemented/verified）、recall 指標、dispatch bypass audit、KPI event id。

【0924 勾選帳補落（同 135.6——批量夜 receipt 宣稱 Done 但卡面帳未落）】correction 迴路四步腳本 b0fb2945 在 main＋parent 批量 receipt 宣稱＋status Done。#6 約束區非謂詞。逐條深驗（KPI 重述率實測等）隨 S7 度量累積。
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
correction 迴路四步全數落地並通過 bi review＋judge 收斂：挖掘（mine_corrections --since/--until 弧窗＋post-build correction mining checkpoint）→載體路由（四載體分流表內嵌 checkpoint 節）→派工前執法（decisions_pending lint --dispatch 精確卡匹配＋work-order 義務一行）→KPI（correction-kpi.jsonl detected→incorporated；restated 身份＝correction id＋原文 hash）。commit 700bbb1f @ ephemeral/air-135.8-impl（trunk merge 待 user 授權）；review＝muse 全 diff＋codex 設計腿 approve-with-findings；機驗 28＋全量 2136 tests 綠。後續項歸 KPI 讀端/135.7 批次（見卡 notes）。

終態圖（as-built）：

```mermaid
flowchart LR
  UC["user correction"] --> MINE["①mine_corrections --since 弧窗<br/>post-build checkpoint Detect"]
  MINE -->|"candidate 非空"| EX["②Extract LLM 判讀<br/>重述比對＝correction id＋hash"]
  EX --> ADD["decisions_pending add<br/>--kind correction（KPI detected）"]
  ADD --> ROUTE["④路由四載體<br/>卡面／decision／rule／skill"]
  ROUTE --> CLOSE["close（KPI incorporated）"]
  ADD -.->|"open blocking"| ENF["③lint --dispatch<br/>派工前禁派工先收編"]
  CLOSE --> S7["重述第二次＝fail→parent S7"]
  ENF --> S7
```

三圖對照：desc 圖（迴路 intent）與 as-built 落點一致——四步各歸其位，零 pivot。
