...
> 額外記錄（09-13 晚）：bridge GLM family 持續 failed-usage（Model creation failed）已排除 app 重啟過渡期——待驗假設：今日 ZCode app 更新（binary 11:49）改變 headless config 要求→bridge staging 相容性壞＝delegate-bridge 側建卡修；AIR-88 probe 同故障域阻塞中。與本 handoff 兩項正交（:227 修宣稱過期，與 bridge 現況故障無關）。
