...
**Memory spine 位置**：跨池共享記憶 spine＝`~/.agents/memory-spine/`（條目由 ai-guide 側 session 寫入、各池唯讀；`index.md`＝位置決議＋各池 routing 認養表——找 spine 條目（如 `model-runtime-entitlements`、slug glossary）先查該索引）。
