...
- **Dispatch⇄collection 配對**：派工必配回收——背景 detach 完成不通知，**waiter exit 即通知**；watcher 主路徑＝`scripts/bridge_waiter.py` 背景 shell fan-in 包 wait（124 內部消化、全 terminal 喚醒輸出 CollectionReceipt、exit 2 fail-loud 禁 retry、exit 3 stalled advisory；契約單一源＝bridge_waiter docstring）；裸 `wait` 背景 shell 降為 fallback（script 缺時；exit 124 仍＝re-arm 非失敗、禁重派）。terminal ≠ complete：有 sink 登記者以 artifact 機驗（存在＋非空＋錨點）為完成，無登記者以 bounded receipt 非空為完成；workflow 層配套（bounded slices／checkpoint 續寫）單一源＝AIR-135.7 契約。
