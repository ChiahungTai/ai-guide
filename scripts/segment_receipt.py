...
機械欄：baseline_head（EP `> **baseline**:` 解析）、head、tracked_diff_hash
（`git diff HEAD` content hash）、untracked manifest（路徑＋content hash，
untracked-only 變更也算內容變更——與 workflow-review-pattern header identity
同詞）。判斷欄（未驗面／下一步／review 尚需）**不住本檔**——寫 EP 進度節，
receipt 只記 EP 指針。
...
CLI：
- 生成：`uv run python scripts/segment_receipt.py --repo <root> --segment <id> --ep <ep.md> [--parent <receipt>] [--pytest-cmd "uv run pytest"] [--pytest-args "-q tests/x.py"] [--out <path>]`
- 驗證：`uv run python scripts/segment_receipt.py --verify <receipt> [--repo <root>]`
...
def compute_identity(repo: Path, ep: Path | None = None) -> dict:
...
def generate_receipt(
    ...
    """
    identity = compute_identity(repo, ep)
    ...
    body = (
        ...
        "> 判斷欄在 EP 進度節——本 receipt 是 validity token，不是第二真相源；\n"
        "> 完成度真相＝Git＋EP re-derive。freshness 核對："
        "`--verify <本檔>`（FRESH＝EP 判斷欄仍有效；DRIFTED＝以實物 re-derive）。\n\n"
...
def verify_receipt(receipt: Path, repo: Path) -> tuple[bool, list[str]]:
    ...
    世界是否已分叉的比對鍵：baseline_head（EP 可達且解析一致）、
    ...
    if data.get("baseline_head") != current["baseline_head"]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--ep", help="EP 檔路徑（baseline 解析來源；repo 相對或絕對）")
