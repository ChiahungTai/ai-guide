...
- `/post-build` — build 後收尾鏈編排（diff triage → code 鏈 [code-review（風險 profile 派發）→ judge-review → 修正迴圈] → docs 鏈 [consistency → metadata-sync → tour corpus 修復閉環] → memory 收尾腿（弧 footprint 盤點＋即時蒸餾/標 terminal；AIR-117） → 殼 refresh [hook 2：實作章節＋產圖一次＋badge ✅＋持久 delta tour] → 收尾報告；止步於 /commit 前）
...
- `agent-workflow` — Agent 派發 / worktree 隔離 / 並發控制 / spawn 預設背景 / spawn 型別 gate（內建 general-purpose／Explore 無 pin 繼承主模型——lite／機械角色任務〔judgment floor=execution〕必派 registry 角色；model/effort 解析＝WorkUnitContract→DispatchPlan）/ 委派框架（delegation）/ side-discovery / Rule Freshness（spawn 時注入）/ Writer-Reviewer / spawn 失敗階梯（429 降並發→serialization）/ **全生命週期 execution contract 消費側**（各段 dispatch 查表；表主體在 agents/AGENTS.md）
- `conversation-dispatch` — 會話主動派工判準（主 agent 討論座席、查證打雜自動外派 sub agent 背景跑）：每查證問句派/不派判準表（heuristic 非 normative——正表：廣度→Explore／機械對帳→lite-verify／逐字規格→spec-miner／多源→cross-verify-investigator，負空間等重：≤2 檔直查／<30s 前台 probe／判讀裁決不外派）／spawn prompt 模板（WorkUnitContext，不倒整段對話）／回報契約（結論＋path:line 逐字錨點＋未驗項分列，錨點失效退回重取）／額度同意邊界（唯讀 in-harness 自主派、bridge/external-runtime 必先問 user）；spawn 機械/路由引用 agent-workflow＋model-routing
...
- `model-routing` — subagent 派工路由深層載體（reference skill：AIR-91 doctrine＝WorkUnitContract schema／Role→authority allow-list／resolver precedence 七步／AvailabilitySnapshot tri-state／ArcOverride／RoutingPolicy／DispatchPlan/Trace schema——model 供給事實單一源＝`skills/model-routing/catalog.toml`〔ModelIdentity／DispatchBinding／qualification〕、registry 部署預設單一源＝`agents/presets.toml`〔default binding FK→catalog〕，值不重列）＋dispatch 預設與額度 failover（policy 面；額度現值查 spine `model-runtime-entitlements`）＋旗艦資格條款（五項）／坐位註記＋內建型別無 pin 繼承（general-purpose／Explore 繼承主 session 模型；execution work unit 誤派內建型別＝旗艦燒機械段反模式）＋lite 分工律〔judgment floor=execution 條件式降級＝保護面厚度三件；decision（判斷密集位）不可降、模型歸因紀律〕＋external-runtime family→(model,effort,容量) 解析表＋eligibility gate／reviewer 交接契約／完成回報收法（fire-and-forget 決策樹：--background 提交＋跨 session 認領＋wait/show 晚收、timeout 訊號家系拆分、ETA-gate fallback）／套用三路徑、rate limit 並發表、thoughtLevel 但書；rule 端留 precedence 骨架＋hard invariants——rule+skill 分層控制 bundle 尺寸）
- `self-contained-prompt` — 交接 prompt 設計原則（接手方三層 / schema / 決策脈絡 / drift / 機密）；/handoff 與 agent-review-cycle 共用
- `memory-audit` — auto memory 稽核/清理（兩級：full 四層=索引量測+內容核實 vs repo+清理+盤點 / lite=git log 增量核實；索引整潔≠記憶健康、內容核實預設必做；狀態戳 `_audit-state.md`；advisory→核可→執行三分離；generator 池層 1 縮為 `--check` 投影驗證——資產 `skills/memory-audit/scripts/generate_index.py`；寫入端紀律＝六問〔首問任務終態→卡〕＋desc 三不＋弧結案蒸餾〔mem-distill 執行形態〕＋rank 排序；body Read 觀測取樣線索＝`memory_telemetry.py reads`〔AIR-41——候選≠可刪、觀測非真值〕；收斂波 git 基線 diff-driven〔波前二分：流入快照/停波〕＋decay 候選清單〔AIR-49——`decay` subcommand 只產候選人裁〕＋歸因投影〔AIR-55——`attribution` subcommand：last_tracked_writer＋dirty_after_tracked〕）
