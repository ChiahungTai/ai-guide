...
2. 全量測試（**用專案命令**如 `make test`，非 bare `uv run pytest`；完成閘門見 [build.md](../implement/SKILL.md) 階段 3）（背景跑）
3. POC/demo 全量驗證
...
### 禁止
- ❌ 交出半成品就停——completion 必掃 user-visible surface AC（見 [kanban-board](../kanban-board/SKILL.md)「Mockup 契約」），surface 項未驗即未完成
- ❌ 遇到錯誤就放棄
- ❌ 使用 `sed` 修改程式碼
- ❌ 多行 `python -c` 中使用 `#` 註解
