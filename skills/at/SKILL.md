...
> - **出路（已關閉——user 0925 裁決不上游）**：牆一＝carrier 端 Patch（app-server 直呼官方 AutomationService，probe 可行 ~3 檔百行級）；牆二＝upstream proposal（CronCreate 序列化點把呼叫端 provider 視角解析成 registry canonical `account:` 形式，說帖定稿存 AIR-195 卡 WT `.agent-tmp/`）。兩條修復路 user 均裁決不走 upstream——本 gap 轉**永久已知限制**：workaround 即前述共通繞道（UI 建排程／UI 重選模型救回），carrier 自行修復前不重評。
