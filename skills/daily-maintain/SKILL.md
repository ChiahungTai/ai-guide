...
| 決策點 | 行為 |
|--------|------|
| 🟢 低風險 findings（X-cap-path） | **自動修正**，不詢問 |
...
理由：用戶透過排程載體排程此命令 = 明確授權自動維護。只有 🟢 項目（路徑修正、tag 修正）會被 commit，🟡 項目絕不 commit。
