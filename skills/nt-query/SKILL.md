---
name: nt-query
description: "Query NautilusTrader v2 (Rust + PyO3 runtime) correctly — docs-first for capability, in-repo AGENTS.md hubs for orientation, LSP on in-package stubs for implementation, designer-intent for usage contract, MIGRATION_V2.md for v1→v2 mapping. Use when investigating what NT v2 supports ('NT 支援/能不能/能力/概念'), v1→v2 migration (import flattening, API renames, behavior changes), where/how v2 implements something (LiveNode, DataActor, BacktestEngine, on_quote/on_bar, cache, portfolio, Rust crates), whether a v2 API is safe in a given context (loop ownership, node run/start/poll/stop), or porting v1 code. Two knowledge layers; confusing them is the most expensive mistake — never write v1 names from memory into v2 contexts. For the legacy v1 Cython runtime use the nt-v1-query skill."
