...
**單一源**:本 skill 是 finalization 邏輯的唯一定義處。三個 invoke 點(build 階段 5 結算／收斂後結案／standalone 補漏)共用,消除「同一邏輯散落多處 → drift」。

## 三 mode(finalization 在 build 結算,commit 純提交)
...
| **backlog 結案** | A（時點＝收斂後） | 已完成 UC 的卡結案兩步＋弧結案蒸餾第三動（本弧 memory 條目終態化）：`task edit <id> -s Done --final-summary`（refs 開工出生即寫，結案補寫可選），卡留 Done 欄（命令合約見 [kanban-board](../kanban-board/SKILL.md)） |
...
| **EP 歸檔** | A, D（時點＝收斂後） | **歸檔前查證（防 ghost-done）**：列 EP 交付物（UC盤點/收尾/各段 deliverable）逐項 rg/fd/Read 驗落地——「段落完成」≠ codebase 真有（曾發生整份 EP 100% ghost-done 誤歸檔）；**card-first 分支**：無 EP 弧以卡 AC/Plan 交付物為對帳源（TaskRef），該弧原有 EP 時照舊；有 ghost-done 不歸檔（補做或標 🔧）。全綠才歸檔 → **結案形態依消費端專案的 task-placement policy**：task directory 為 **identity-stable**（專案宣告路徑永不搬）→ 歸檔即卡 status 翻 Done，無 filesystem relocation（ai-guide AIR-77 起屬此類）；專案慣例為 **archive-relocation**（歸檔＝目錄整搬）→ 才 **task 目錄整搬**（目錄級非單檔 mv；EP/spec/Report Shell 同目錄一起走）至 **任務家下 repo 既有歸檔慣例**：任務家探測（card-first resolver——單一源見 [illustrate html-mode](../_common/illustrate-html-mode.md)「產物位置分流」：`ai-analysis/_tasks/` 在場→雜項家；線任務 EP→`ai-analysis/_projects/<線>/`、歸檔落同線 `done/`；否則 repo-root `00-tasks/`），其下探測 `done/` 或 `_done/`（含 `_done/<YYYY>/` 年分層——存在則搬入當前年層）任一存在者沿用，兩者並存沿用最近歸檔落點；皆無 → 建任務家下 `done/`（跨專案 skill 不 hardcode 單一歸檔形態——曾 hardcode `_done/<YYYY>/` 與消費端 `done/` 慣例漂移，照 skill 走會建出第二歸檔目錄；舊 `ai-analysis/execution-plans/` 慣例退役）;綱要 EP(blueprint)等所有衍生子 EP 完成才歸檔 master |
...
| 漏項 | 觸發訊號(變更檔集模式) | 窄驗證 |
|------|----------------------|--------|
| EP 歸檔漏 | EP 段落交付物已 commit(code/test)且任務家對應 task 目錄仍未歸檔（未搬歸檔目錄；ai-guide 本 repo 無搬移制——此漏項僅外 repo；**card-first 分支**：無 EP 弧＝卡 AC/Plan 交付物已 commit、卡仍非 Done） | `fd -e md -E done -E _done -E _archived . <任務家>`（存在的任務家：`ai-analysis/_tasks/`、`00-tasks/`；線任務另查 `fd -e md . ai-analysis/_projects/*/tasks/`；resolver 單一源＝[illustrate html-mode](../_common/illustrate-html-mode.md)「產物位置分流」）比對已交付未歸檔 |
...
> **原子性**:build 情境 A 分兩時點原子——5a Built 結算三件「Capabilities 寫入 + 消費場景寫入 + SYSTEM-MAP 預覽」同時；收斂後 final 結案四件「backlog 結案兩步 + SYSTEM-MAP 升級 + EP 歸檔 + flow-feedback 歸檔」同時（各時點內部分完成 = 狀態不一致誤導 LLM）。architecture.md 更新 / consistency 閘門隨 5a（各自獨立條件時序，與三件平行）。standalone 補漏/更新不要求原子(補的是各自獨立的漏項)。
