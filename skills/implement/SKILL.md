...
**生成面判定（步驟 1 前置——5b 生成分支／AGENTS coverage，AIR-164）**：`git diff --diff-filter=A --name-only` 聚合本弧新增檔到頂層目錄，逐 candidate 對 MUST predicate 判定——**predicate 三態（MUST／MUST NOT／CONDITIONAL）與 owner 認定的規範源＝[instruction-writing](../instruction-writing/SKILL.md)「instruction surface coverage」節**，本段不重列（引用語義以該節為準）。機械觸發面：命中任一且**無 owner**（該目錄無 AGENTS.md；root AGENTS.md 已明示擁有該面即 owner 已滿足，免建 local 面）→ **create-or-sync 生成**（骨架四要素與建檔閾值照 [instruction-init](../instruction-init/SKILL.md)；雙檔模式補 CLAUDE.md `@AGENTS.md` wrapper）；目錄**已有** local AGENTS.md → 不走生成分支，照下方步驟 1-4 更新路徑（不變）。零新增檔 → 判定空跳。此分支與 [post-build](../post-build/SKILL.md) 階段 4「AGENTS coverage gate」共用同上規範源——build 側主動生成，post-build 側兜底反掃。
