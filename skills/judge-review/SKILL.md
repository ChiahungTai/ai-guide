...
5. **輸出評估報告**（格式如下）
6. **寫入持久化**：決策更新到 finding 的 `decision`(✅/❌/⚠️)與 `status` 欄 —— ✅→`adopted`、❌→`rejected`、⚠️→`needs-confirmation`。帳本＝**更新呼叫端指定帳本（鏈上預設 `.review/<branch>.md`＝工作帳本；EP review 區段＝規劃期帳本，僅服務 EP Review Cycle）**。`.review` 是 local-only（gitignore；同 worktree 跨 session 保留——post-build Resume 依賴此、跨 branch/worktree 不保留；隨 commit 清除——晚到 findings 的 durable 落點由收尾殘留清單承載）。格式見 [workflow-review-pattern.md](../_common/workflow-review-pattern.md)。**結論回卡（強制——AIR-108）**：裁決完成當下以一行 `backlog task edit <卡id> --append-notes "judge verdict 摘要（✅N/❌N/⚠️N＋evidence ref/jobId）"` 落卡——`.review` 隨 commit 清除、卡面是跨 session 唯一 durable 驗證狀態源（findings 不落卡＝下個 session 考古，SC-32 實證）；無卡弧落 EP notes／report 並在收尾報告記錄去向。**judge-review 不實作** —— 實作由呼叫端決定(`/implement` Phase 4 直接 apply ✅;獨立使用由用戶判讀決策清單)
