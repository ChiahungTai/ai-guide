...
3. **無特權路徑＋隔離 authoring（card WT 隔離——AIR-106）**：author session（含 marshal 直編）不豁免。**canonical 主樹（primary WT）留在 main**——控制面 authoring 走 **persistent card WT**（`scripts/wt-open.sh <卡id> --base main`）或任一非 canonical WT：`~/.claude/rules/` 與 skills symlink 指 canonical working tree，primary 切卡 branch＝checkout 即生效（F8 的 checkout 形態——與 commit 無關）。機械防線＝`.githooks/pre-commit` → `control-plane-guard.sh` 擋「canonical main 上控制面路徑 commit」（路徑清單單一源＝該 guard 的 grep pattern 行；豁免①②③〔backlog metadata／結案〕不受影響；④ style-only 觸控制面路徑時仍走卡 branch）。**guard 防誤落直 commit，不審 merge 落地**——`merge --ff-only` 不產生 commit、不觸 hook；merge 前回執覆核＝commit skill 階段 2.9（commit 先於 merge，2.9 即最後覆核點——單一承接點，precheck 無回執職責）。各 clone 需 `git config core.hooksPath .githooks`——未設＝閘未上線（fail-open，提案須揭露）；detached HEAD 亦在閘外（事後 `branch -f main` 同繞過徑——與 `--no-verify` 同信任等級，如實揭露）。逃生口＝`--no-verify`（同時跳過測試閘——手動補跑；須記卡 notes）。
