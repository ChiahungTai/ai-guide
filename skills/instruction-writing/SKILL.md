...
4. **豁免**：純 typo／link／格式／零語義差（由 diff 自證行為等價）走 static-only。**行數不是判準**——一個 modal word（MUST→SHOULD）可以是高影響控制面變更。豁免免的是審查腿，**不免隔離 authoring 與部署對帳探針**。
5. **回執（四欄正典——AIR-105 原型晉升）**：結算記錄 `classification=<profile>／review=<腿＋evidence ref；static-only 豁免填 static-only（diff 自證）>／session-freshness=<fresh｜stale＋重讀證據——author session 結算自報，隨 Receipt 行入卡 notes>／deployment-surfaces=<healthy｜unhealthy｜pending｜unverified｜N/A>`；消費端格式＝commit 報告（commit skill 階段 2.9）與 post-build 收尾報告——缺回執不得宣稱收斂。
