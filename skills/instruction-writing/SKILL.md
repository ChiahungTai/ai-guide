...
3. **無特權路徑＋隔離 authoring（card WT 隔離——AIR-106）**：author session（含 marshal 直編）不豁免。**原則跨 repo 通用：控制面 authoring 與 canonical／owning 線隔離，隔離機制與主線名由各 repo AGENTS 自訂**——本節以 ai-guide adapter 為例（canonical 主樹＝primary WT 留 `main`；ai-guide 專屬機具＝`scripts/wt-open.sh`／`.githooks/control-plane-guard.sh`，無此機具的 repo 退為純 LLM 紀律）：**canonical 主樹留在 main**——控制面 authoring 走 **非 canonical WT——persistent card WT（`scripts/wt-open.sh <卡id> --base main`）或等價的獨立 worktree**：`~/.claude/rules/` 與 skills symlink 指 canonical working tree，primary 切卡 branch＝checkout 即生效（F8 的 checkout 形態——與 commit 無關）。機械防線＝`.githooks/pre-commit` → `control-plane-guard.sh` 擋「canonical main 上控制面路徑 commit」（路徑清單單一源＝該 guard 的 grep pattern 行；豁免①②③〔backlog metadata／結案〕不受影響；④ style-only 觸控制面路徑時仍走卡 branch）。**guard 防誤落直 commit，不審 merge 落地**——`merge --ff-only` 不產生 commit、不觸 hook；merge 前回執覆核＝commit skill 階段 2.9（commit 先於 merge，2.9 即最後覆核點——單一承接點，precheck 無回執職責）。各 clone 需 `git config core.hooksPath .githooks`——未設＝閘未上線（fail-open，提案須揭露）；detached HEAD 亦在閘外（事後 `branch -f main` 同繞過徑——與 `--no-verify` 同信任等級，如實揭露）。逃生口＝`--no-verify`（同時跳過測試閘——手動補跑；須記卡 notes）。
4. **豁免（static-only——機械 predicate，非散文判準）**：入場＝deterministic 可證行為等價的 transformation 白名單，僅兩類：①whitespace／markdown 格式重排——範圍限 markdown body（frontmatter 全區、code span、machine-consumed region 一律不入場），且**作者證明**渲染輸出 byte 等價（未證明＝無法求值→boundary）；②非 code token（fenced／inline code span、命令、檔名裸寫）、非 frontmatter（key 與 value 全區）、非路徑（含 URL）／identifier token 的純 prose 錯字修正，且 diff 前後**詞彙**編輯距離 ≤2（word-level Levenshtein，Unicode NFC 正規化、大小寫敏感、逐 diff 計算）；**否定詞、數值、單位、日期、模態詞的增刪改一律不視為錯字**（收窄列舉，疑義即落袋）。**本判準僅適用 instruction 條文（markdown）之 hunk**——code／config／test 等其他檔型 hunk 一律不入場，逕依第 1 款分類。**typo 觸及 code token／frontmatter／路徑／identifier token／模態詞行 → 退出 static-only，回第 1 款依 review-engine 正常分類**（模態詞行＝行粒度連坐——寧濫勿缺；模態詞表單一源＝post-build「純修飾快道機械排除」詞表）；link URL 修正不入場（走 standard／boundary）；分類無法求值 → 直接 boundary（fail-closed）；分類有爭議 → 走全套。**行數不是判準**——一個 modal word（MUST→SHOULD）可以是高影響控制面變更。豁免免的是審查腿，**不免隔離 authoring 與部署對帳探針**。
5. **回執（四欄正典——AIR-105 原型晉升）**：結算記錄 `classification=<profile>／review=<腿＋evidence ref；static-only 豁免填 static-only（diff 自證）；外審零容量 no-candidate 填 deferred:<pending-ref>（帳本引用）>／session-freshness=<fresh｜stale＋重讀證據——author session 結算自報，隨 Receipt 行入卡 notes>／deployment-surfaces=<healthy｜unhealthy｜pending｜unverified｜N/A>`；**`review=deferred` 語義＝non-terminal pending 的記帳標準化——no-candidate 帳本（review-engine 執行預設）仍為 authoritative pending record，receipt 值僅為其 projection、不取代帳本；deferred 對一切 consumer（含 commit 階段 2.9、merge 回執覆核）＝present-but-blocking（landing-ineligible）：不滿足 landing gate、不得落地。補審 SLA＝額度恢復偵測日（偵測點＝既有派工前 availability 查核——窗口語義正典）起 N=7 曆日內補腿（凍結值，變更走 EP amendment）；recovered_at／due_at 記入帳本重查條件欄；逾期＝維持 pending＋續禁落地（fail-safe），留帳本 open 項。deferred 相對既有 pending 僅增加回執顯式化與 SLA 可追蹤兩點，不生其他新語義；boundary baseline 不變**。消費端格式＝commit 報告（commit skill 階段 2.9）與 post-build 收尾報告——缺回執不得宣稱收斂。

已知威脅（防儀式化）：外審容量緊時「降級分類」是理性選擇——fast-track（static-only）必須保持機械可判定不得擴張；分類有爭議時 author 提案、reviewer／judge 複核。
...
> skill 完整 frontmatter 欄位（`when_to_use` / `paths` / `disable-model-invocation` / `context:fork` / `allowed-tools` 等）與決策原則見 [skills/AGENTS.md](../AGENTS.md)（單一真相源，此處不重複）。
...
> **核心理念**: instruction file 是模組知識的 Encoder（壓縮表示）。品質標準是 Signal/Noise ratio — 保留從程式碼猜不到的知識，移除可推導的內容。詳細框架見 [encoder-philosophy.md](../_common/encoder-philosophy.md)。
...
- **設計理由（預設精煉一句）**: 為什麼這樣做而非那樣做 —— 一句 why 即可（防 LLM 不懂而亂改）。**例外**：規則易被 LLM/社群慣例動搖（LLM 會 rationalize 違規，如「加統計無妨」）→ 整段反駁論證是防亂改的疫苗而非贅述，段落標 `補充論證`/`第一性原理`，先承認反方價值再反駁（範例見 python-standards Facade Pattern 段）
- **架構約束**: 不可妥協的設計限制
...
| `## 模組導航` | 概念→符號種子（導航-A）。**預設用 per-concept 內嵌描述**（符號旁帶一句職責）；符號多時用 `### 類別小標題` 分組（非 Class→檔案映射表）。**禁止 Class→檔案集中映射表** —— 前兩欄 LSP 可推導（見「應該避免」條目），必然 drift | 有公開符號 |
| `## 可複用基礎設施` | 跨模組共用 symbol（格式見上方「可複用基礎設施段落規範」） | 有跨模組消費者 |
| `## 模組檔案結構` | 目錄樹（code block 格式，`檔案:symbol` inline 註釋）。**禁止 markdown table 格式**（`| 檔案 | Class | 職責 |` = Class→檔案映射表）—— table 形式是映射表，code block 形式才是目錄樹 | 檔案數多到值得展示組織 |
...
> 以下論證回答「為什麼統計資訊/版本號/更新日期對 AI 無價值」——規則易被「加個統計無妨」rationalize 動搖，整段反駁是防亂改的疫苗（instruction-writing「設計理由」條目的例外模式）。

### 為什麼統計資訊對 AI 無價值？

- **時點性問題**：行數是程式碼變動的快照，每次修改都會過時，AI 讀取時行數已經不同
- **認知負擔**：AI 看到 `行數: 100` 可能以為是小檔案，實際可能有 10000 行在檔案其他地方 → **造成錯誤判斷**
- **維護成本**：每次修改都要更新統計，實用價值接近零

對 AI 真正有價值的資訊：**檔案存在性**（檔案在哪裡）、**職責劃分**（每個模組做什麼）、**設計理由**（為什麼這樣做，從程式碼猜不到的）、**關鍵約束**（不可妥協的規則）、**引用路徑**（相關檔案位置）。
...
正確做法：功能性描述取代版號（`ClassName`（pure AND/OR evaluator, multi-setup, YAML-driven）而非 `ClassName（v7: ...）`）。唯一例外：API 中的實際方法名或常數名包含版號時（如 `full_pipeline_v2()`），如實引用。

**結論**：移除統計表格是正確決策 — 減少 token 消耗、避免 AI 被誤導、專注於當前有效的規則。
