...
3. **無特權路徑＋隔離 authoring（card WT 隔離——AIR-106）**：author session（含 marshal 直編）不豁免。控制面修改在**卡 branch（`air-<N>`）或 persistent card WT** authoring——canonical main 上控制面路徑 commit 由 `.githooks/pre-commit` → `control-plane-guard.sh` 機械擋（路徑清單單一源＝該 guard；開工／建卡／結案 metadata 例外只觸 `backlog/` 不受影響；逃生口＝`--no-verify` 須記卡 notes）。審查腿對 branch 態跑，**回執四欄齊後才 merge canonical**——Claude 端 `~/.claude/rules/` 為 live symlink，canonical 一改即對 Claude session 生效，「merge 前審查」才等於「生效前審查」（F8 反例：AIR-105 commit 先於審查腿回收＝activation-before-review 洞）。
...
5. **回執（四欄正典——AIR-105 原型晉升）**：結算記錄 `classification=…／review=…（evidence ref）／session-freshness=…／deployment-surfaces=…`；消費端格式＝commit 報告（commit skill 階段 2.9）與 post-build 收尾報告——缺回執不得宣稱收斂。
