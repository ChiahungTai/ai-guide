...
所有 rules/skills/commands 的**文件本身供 AI 消費**（AI 讀 `.md` 來執行命令）—— 寫作、審查、修改以「AI 能否正確執行」為準。readability 對 AI = 結構可機械解析、指令可遵行，**不是人類閱讀流暢度**；禁止用「人類讀者需要前置框架/會困惑」這類人類認知論證當審查發現（作用域限軌道① LLM 執行鏈命令——軌道② viewport 命令〔debrief/illustrate/smell-detector〕的產出本以人類可判讀為目的，人類認知論證是其正當審查準據）。無需人類式證據出處、版本履歷、精確專案數字（詳見 instruction-writing skill、`rules/instruction-writing.md`）。
...
- **review 模式**：Writer/Reviewer 分離——獨立 context 審查（同 LLM 自審有 bias → spawn fresh reviewer；第二意見升級軸＝跨家族）
...
- **review 鏈**：review command 支援「產出 finding → 跨 context／跨家族貼回 → `/judge-review`」持久化鏈（durability 基建，與 session 邊界脫鉤）
...
- **生命**：開工（implement 階段 1 起手式 ⑤ 後——⑤ 定義＝[kanban](skills/kanban-board/SKILL.md) 起手式五步）自 main `git checkout -b air-<N>`；branch 已存在→先 `git log main..<branch>` 判本卡遺留再續用（非本卡殘留不續用）；建卡不開 branch；卡 branch 上 commits message 帶卡 id（`(air-46)` scope 或 `air-46: …` 前綴）；**控制面弧形態依「控制面隔離閘」條**——authoring 走非 canonical WT，primary 切卡 branch＝live symlink 即時生效面（不適用本條 legacy checkout）
- **收尾**：二層邊界（AIR-183＋AIR-200 修訂）——卡 branch commit＋本地 trunk merge（ff-only）＝receipt-predicate 委任（互動＋批量同制；predicate 單一源＝[outward-action-consent](rules/outward-action-consent.md)「Commit 專屬段」＋[commit skill](skills/commit/SKILL.md)「互動 receipt-gate 驗收程序」；merge 前置＝fresh receipt＋ff-only＋控制面 landing eligible，rebase 即重驗；merge 後 main 崩＝marshal 立即 revert＋揭露）；push 恆停；卡 Done 翻牌拍板仍 user（結案兩步）。/commit 確認通過後——`git checkout main && git merge --ff-only air-XX && git branch -d air-XX`（WT 回 main）；`--ff-only` 被拒＝main 已前進→先 `/rebase main` 再吸（mosaic：`/rebase <owning線>`，禁默認 main）；clean 紀律由 /commit gate 守（dirty checkout 不衝突會帶走，不是護欄）；trunk 永不被 rebase、永不 force（[rebase skill](skills/rebase/SKILL.md) 鐵律）；**控制面弧 merge 前回執四欄須齊**（[instruction-writing skill](skills/instruction-writing/SKILL.md)「落地前審查閘」——guard 不審 merge 落地）
- **WT 形態過渡條款（09-16 AIR-72 起雙軌）**：卡可走 **persistent card WT**（control/execution plane 分離）——開工＝`scripts/wt-open.sh <卡id> --base main`（只接已存在 card，不代建卡不配 id；**base 顯式＝owning 線 ref，絕不取呼叫端 HEAD**；`--base` 無預設值，一律顯式傳 owning 線；機械建立 `<repo-basename>-<branch>` WT＋池/inbox symlink→primary canonical＋identity contract（`<wt>/.agent-tmp/wt-identity.json`）＋{toplevel,branch,card,baseline} 驗證），收尾＝`scripts/wt-close.sh --wt <path> --preflight` 全檢查零變更，trunk 收斂由 marshal 收線時 full 執行（AIR-200：自主——rebase/ff-only 規則同「收尾」條；**full＝rebase→merge→移除 WT 目錄並刪 branch——收線後目錄/branch 殘留即異常**（air-68 實證）；免卡小修走 `--ephemeral <name>` fast-path）。**未走 wt-open 的卡照舊**：上兩條 checkout 模式仍是 runtime authority；同一卡禁混用兩形態（primary 已切卡 branch 又開同卡 persistent WT＝ownership 不唯一）。WT 形態下 board single-writer 生效——卡 metadata 只有 board-control 可寫、spawned worker 禁碰（單一源＝[kanban skill](skills/kanban-board/SKILL.md)「board single-writer」）；形態定案與 wt-open/close transaction 規格＝[blueprint workflow.md](ai-analysis/blueprint/workflow.md)
- **控制面隔離閘（AIR-106）**：控制面 authoring 走 **非 canonical WT——persistent card WT（`scripts/wt-open.sh`）或等價的獨立 worktree**——canonical 主樹（primary WT）留 main：`~/.claude/rules/` 與 skills symlink 指 canonical working tree，primary 切卡 branch＝checkout 即生效（F8 的 checkout 形態）。機械防線：canonical main 上控制面路徑 commit 由 pre-commit `control-plane-guard.sh` 擋（**路徑清單單一源＝該 guard 的 pattern 行，本檔不重刻**；豁免①②③不受影響、④ style-only 觸控制面路徑仍走卡 branch）；guard 只防誤落直 commit、**不審 merge 落地**——merge 前回執覆核由結案兩步＋commit skill 階段 2.9 守。各 clone 需 `git config core.hooksPath .githooks`（未設＝閘未上線）；逃生口 `--no-verify` 須記卡 notes；why＋流程細節＝[instruction-writing skill](skills/instruction-writing/SKILL.md)「落地前審查閘」節
- **`/rebase all` 護欄**：all 之前 `git branch --list 'air-*'` 非空 → 先收卡，不跑 all（`*` 不可省）；user 紀律：all 只在所有卡收尾後下
- **軟條款**：忘記 checkout 直落 main＝軟失敗不回頭搬（message 已帶卡 id）；**控制面路徑不適用軟條款**——被 guard 擋下＝走卡 branch 重做（上條）；夜間 automation commits 落卡 branch 無害；拋棄（`branch -D`）需 user 明確確認；跨 session 遺留屬正常（接手者續用或判死吸收）
- **mosaic 變體**：多線形態（前綴＝WT toplevel basename 縮寫、owning 線由卡 labels 線 tag 判（v2→v2 線、warrants→warrant 線、其餘含無線 tag＝main）、線判定路徑查表、收尾 rebase base＝owning 線）定義源＝[09-08 卡 branch EP](ai-analysis/_tasks/_archived/09-08-card-branch-lifecycle/ep.md)——落地由 mosaic 側 MOS 卡承接
...
| 層 | 機制 | 獨立性 | 軸 |
|----|------|--------|-----|
| 1 | 獨立 context LLM 自判（agent review＝fresh subagent context——同 session 不同 context；`/ep-review` in-pipeline、`/audit-test`） | 低 | A |
| 2 | 跨家族第二意見（`kind=different_provider_family`，經 delegate-bridge `--family muse|codex` 跑 `/code-review`/`/ep-review`，findings 貼回實作 LLM → `/judge-review`） | 中 | A |
...
| `/ep-review` | LLM | 1（獨立 context 自判）/ 2（跨家族） | AI-self / 人類（跨家族） |
...
工具/維護命令（`/doc-health`、`/consistency`、`/instruction-*` 等）與受眾模型正交，完整索引見 `skills/AGENTS.md`（source；`skills/CLAUDE.md` 僅 @wrapper）。
...
- `skills/` — 領域知識和工作流 skills（on-demand；SKILL.md 開放標準，跨 harness 可攜；Claude 端 `/name` slash 與 Skill tool 皆可觸發，工作流 skills 索引見 `skills/AGENTS.md`）。**reference skill 分層**：on-demand 級 rule 內容下沉至此——rule 留 always-on 核心＋pointer，深層住 `skills/<同名>/SKILL.md` 經全域 symlink 四 harness 按需可讀；這是控制 bundle 尺寸的既定模式（先例：acceptance-evidence / symbol-query-routing / instruction-writing）。**安裝拓撲**：skills 經 `governance/manifest.toml`〔surfaces.skills〕symlink 母鏈（`~/.agents/skills`、`~/.claude/skills` → repo `skills/`）送達各 harness；rules 經 `scripts/deploy_agents.py` bundle（`~/.zcode`／`~/.codex`／`~/.config/muse` 三面；CC 端 rules 走 `~/.claude/rules` symlink auto-load）；plugin 形態（muse-plugins、delegate-bridge）僅用於 symlink/bundle 不可達的機制（gating 閘、跨機 marketplace）
- `skills/_common/` — 共用子範本（跨 skill 引用單元，非 skill；`instruction-*` 等使用）
- `hooks/` — Hook 實作腳本（跨 Claude/ZCode 單一來源。hooks 無目錄載入點，**不能 symlink**——兩家 config 以絕對路徑引用本目錄腳本：Claude `~/.claude/settings.json`；ZCode 3.7.7+ user-level hooks，註冊模板 `governance/registrations/zcode.json`——**模板內容是 `~/.zcode/cli/config.json` `hooks:` 鍵下的子樹值，merge 進去而非整檔覆蓋**（整檔覆蓋會毀掉 config 的 mcp/plugins 區塊）、`notification.sh` 不移植。**跨 harness 治理安裝單一源＝`governance/`**（manifest＋`install.py`，五面 install/check/uninstall——AIR-116）；**新機器全裝入口＝`scripts/bootstrap.py`**（冪等編排器 AIR-110——preflight→installer→approve 暫停點→verify，面外清單與總覽＝[hooks/MULTI-MACHINE.md](hooks/MULTI-MACHINE.md)）。詳細實測與 ZCode 限制（事件子集、專案層忽略、per-session 快照）見 [04 報告 §7 修訂](ai-analysis/reports/_done/superpowers/04-multi-harness機制對照.md)）
...
- `ref-docs/` — 參考文檔（外部書籍 PDF + 衍生分析）；PDF 受版權不 commit（`.gitignore` `ref-docs/*.pdf`）。`ref-docs/harness/` 是四家 harness 官方文檔鏡像（claude-code/zcode/codex/meta〔Muse Code〕）+ `contracts.md` 對照分析——**更新鏡像用既有工具 `ref-docs/harness/crawl.py`**（`uv run python ref-docs/harness/crawl.py [--source zcode]`，discover + sha256 增量寫入 + manifest 維護；不要手動逐頁鏡像）；鏡像**新增／更新／退役**的判準與操作流程＝[ref-docs/harness/LIFECYCLE.md](ref-docs/harness/LIFECYCLE.md)（退役五處連動＋殘留掃描驗證）
...
2. **選對載體**：Hook？Rule？Skill？Prompt/LLM 流程？判準——hook 資格三判準**三者皆是**才選、缺一即退 LLM 流程（假確定性比真語義危險；三判準內容單一源＝[memory-audit](skills/memory-audit/SKILL.md)「載體統一定義表」，資格論證與對照組見 [instruction-writing](skills/instruction-writing/SKILL.md)「載體選擇」）；rule＝須在首個有後果決策前在場的最小核心/bootstrap（always-on 預算稀缺；資格公式見 memory-audit 統一定義表）；skill＝on-demand 方法論（理論深掘、失敗案例群、撰寫規範細則＝reference 分層）；語義判斷（有「看情況」例外）＝prompt/LLM 流程。「該寫哪」統一定義表（載體職責×常駐-按需×寫入預設）單一源見 memory-audit skill；`deploy_agents.py` 的 90KiB gate 撞線時以此為處方
