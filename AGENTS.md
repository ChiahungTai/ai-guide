...
- **收尾**：/commit 確認通過後——`git checkout main && git merge --ff-only air-XX && git branch -d air-XX`（WT 回 main）；`--ff-only` 被拒＝main 已前進→先 `/rebase main` 再吸（mosaic：`/rebase <owning線>`，禁默認 main）；clean 紀律由 /commit gate 守（dirty checkout 不衝突會帶走，不是護欄）；trunk 永不被 rebase、永不 force（[rebase skill](skills/rebase/SKILL.md) 鐵律）；**控制面弧 merge 前回執四欄須齊**（[instruction-writing skill](skills/instruction-writing/SKILL.md)「落地前審查閘」——guard 不審 merge 落地）
