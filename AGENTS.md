...
- NT 查詢工具鏈 — `nt-query`／`nt-v1-query` skills：調查 NT（v2／v1）runtime 行為、遷移或除錯前載入（此行＝兩支 skill 在 ZCode 端的觸發錨——AIR-107；觸發路徑判讀見 instruction-testing「per-harness activation 判讀基準」）
