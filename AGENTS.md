...
- **控制面隔離閘（AIR-106）**：canonical main 上**控制面路徑** commit 由 pre-commit `control-plane-guard.sh` 機械擋（路徑清單單一源＝該 guard：rules／skills／agents／hooks／deploy／muse-plugins／.githooks、AGENTS.md／CLAUDE.md 家族、ai-development-guide.md；backlog／scripts／ai-analysis 不在閘面）——控制面 authoring 走卡 branch，審查腿＋回執四欄齊後才 merge canonical；why 與流程細節＝[instruction-writing skill](skills/instruction-writing/SKILL.md)「落地前審查閘」節；逃生口 `--no-verify` 須記卡 notes
